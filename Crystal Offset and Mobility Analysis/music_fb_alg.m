...
function [dist_MUSIC,spec_dB,peakwidth] = music_fb_alg(M,nsig,Nptot,delta_f,dists,c,n,cal_dist,...
    threshold,CFR_data)
...
% [eigenvects,eigenvals_diag] = eig(RSM);
...
sig_eigenvects = eigenvects(:,1:nsig);
...
S = zeros(M,length(dists));
for i = 1:length(dists)
    S(:,i) = exp(-1i*2*pi*delta_f*(dists(i)/c)*n(1:M));
end
...
% Count the number of significant paths detected
num_paths = length(dist_MUSIC);
